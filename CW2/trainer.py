...
class BaseTrainer:
    ...
    def __init__(
            ...
            X_val: np.ndarray, Y_val: np.ndarray, early_stopping = True) -> None:
        """
        ...
        for epoch in range(num_epochs):
            ...
            for X_batch, Y_batch in iter(train_loader):
                ...
                if global_step % num_steps_per_val == 0:
                    ...
                    # Copied early stopping from CW1 trainer.py
                    print("val_loss", val_loss)
                    print("accuracy_val", accuracy_val)
                    print("train_loss", train_history["loss"][global_step])
                    loss_list = [i[1] for i in
                                 sorted(list(val_history["loss"].items()),
                                        key=lambda x: x[0])]
                    if (len(loss_list) >= 51
                            and not any([i < loss_list[-51]
                                         for i in loss_list[-10:]])):
